  ...
  attributeToSQL(attribute, options) {
    ...
    if (attribute.references) {
      ...
      if (options && options.context === 'addColumn' && options.foreignKey) {
        ...
        const fkName = this.quoteIdentifier(`${tableName}_${attrName}_foreign_idx`);

        template += `, ADD CONSTRAINT ${fkName} FOREIGN KEY (${attrName})`;
      }

      template += ' REFERENCES ' + this.quoteTable({tableName: attribute.references.model, schema: attribute.references.schema});
